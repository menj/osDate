...
if (isset($_REQUEST['start_dateYear']) && isset($_REQUEST['start_dateMonth']) && isset($_REQUEST['start_dateDay']) ) {
	$_SESSION['advsearch']['start_date']   = $start_date = $_REQUEST['start_dateYear']."-".$_REQUEST['start_dateMonth'] .'-'. $_REQUEST['start_dateDay'];
}

if (isset($_REQUEST['end_dateYear']) && isset($_REQUEST['end_dateMonth']) && isset($_REQUEST['end_dateDay']) ) {
...
if (isset($_REQUEST['advsearch'])  ){
	...
	if ( isset($_REQUEST['srchblogbody']) && trim($_REQUEST['srchblogbody'])  != '' ) {
	...
	if ( isset($_REQUEST['srchblogtitle']) && trim($_REQUEST['srchblogtitle'])  != '' ) {
	...
	if ( isset($_REQUEST['srchusername']) && trim($_REQUEST['srchusername'])  != '' ) {
	...
	$blog->sort_page_values = array(
		...
		'date_posted'    => (isset($_REQUEST['date_posted']) ? $_REQUEST['date_posted']:''),
	...
	$t->assign('rendered_page', $t->fetch('admin/blogmatch.tpl') );
...
$strdate = $blog->getOldestDate('admin',$_SESSION['AdminId']);

if ($strdate == '') $strdate = date('y-m-d',time());

$t->assign("starttime",strtotime($strdate));
$t->assign("endtime", time());

$t->assign('addtional_javascript', $js);
$t->assign('addtional_css', $css);

$t->assign('frmname', 'frmBlogsearch');

$t->assign('rendered_page', $t->fetch('admin/blogsearch.tpl') );
