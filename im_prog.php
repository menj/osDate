...
switch (trim($_REQUEST['a'])) {
	...
	case 'sendMsg':
		...
		$msg = $osDB->getOne('select count(id) from ! where recipientid = ? order by sendtime', array(INSTANT_MESSAGE_TABLE, $_SESSION['UserId']));
		$max_cnt = ($config['im_totmsg_count']>0)?$config['im_totmsg_count']:100;

		if ($msg >= $max_cnt) {
			$osDB->query('delete from ! where id = ?', array(INSTANT_MESSAGE_TABLE, $msg[0]['id']) );
		}
