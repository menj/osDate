...
class luckySpinGender extends modPlugin {
   ...
   * @access private
   */
   var $plugin_class_name = "luckySpinGender";
   ...
   */
   function  displayPluginPage() {
	  $this->displayGenderCol($_REQUEST['gender']);
   }
   /**
  ...
  function displayGenderCol($spinGender) {
	...
	if (count($luckySpinRecs) > 0) {
		...
		$_SESSION['luckyspin_id_'.$spinGender] = $data['id'];

		$this->modSmartyAssign('luckygender', $data);
//    $this->modSmartyAssign('spin_again', "?");

		$this->modSmartyAssign('lang', $this->modGetLang() );
		$this->modSmartyAssign('config', $config );
		...
		$this->modSmartyAssign('spinGender', $spinGender);

		return $this->modSmartyFetch('profilebox_gender.tpl');
	} else {
		return '';
	}
  }
