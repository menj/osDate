...
<div class="module_detail_inside top_margin_6px" style="width:100%">
	{assign var="page_hdr02_text" value='<a href="plugin.php?plugin='|cat:$plugin_name|cat:'&amp;do=showquestions&amp;section='|cat:$sid|cat:'" class="subhead">'|cat:$lang.show_questions|cat:'</a> > '|cat:$lang.edit_question}
	...
	<table cellspacing="{$config.cellspacing}" cellpadding="{$config.cellpadding}" width="100%">
		...
		<tr>
			...
			{if $error==1}
				...
				<br/>
			{/if}
				{$lang.question}<font color="{lang mkey='required_info_indicator_color'}">{lang mkey='required_info_indicator'}</font>: <textarea cols="75" rows="3" name="question">{$question }</textarea><br /><br />
				{$lang.descr}<font color="{lang mkey='required_info_indicator_color'}">{lang mkey='required_info_indicator'}</font>: <textarea cols="75" rows="3" name="descr">{$descr }</textarea><br/>
				Type: {$lang.$type}<br/>
