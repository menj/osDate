...
<div class="module_detail_inside">
	...
	<table width="100%" border="0" cellpadding="($config.cellpadding}" cellspacing="{$config.cellspacing}">
		<tr>
			...
			<td width="65%">
				<input type="text" class="textinput"  name="authorizenet_ccowner"/>
			</td>
		</tr>
		<tr>
			<td>{lang mkey='cc_number'}</td>
			<td><input type="text" class="textinput"  name="authorizenet_ccnumber"/></td>
		</tr>
		<tr>
			<td>{lang mkey='cc_exp_date'}</td>
			<td>
				{html_select_date_translated display_days=false end_year=+10 prefix="authorizenet_exp_"}
			</td>
		</tr>
		<tr>
			<td>{lang mkey='cc_cvv_number'}</td><td><input type="text" class="textinput"  name="paypal_cvvnumber" size="4" maxlength="3" />&nbsp;{lang mkey='cvv_help'}</td>
		</tr>
	</table>
	{/if}
</div>
