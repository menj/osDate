...
<div style="vertical-align:top;" >
	...
	{* DIV definition is creating issues of alignment. Hence table defs kept
			{ if $list }
				<div class="module_detail_inside">
					...
					<div class="column_head" >
						<div style="width:7%; display:inline; float:left; text-align:center;">
							<input type="checkbox" name="chkall" value="" onclick="checkAll(this.form,'delete[]',this.checked)" />
							<input type="hidden" name="act" value="{$act}" />
						</div>
						<div style="width:7%; display:inline; float:left; text-align:center;">
							{lang mkey='blog_number'}
						</div>
						<div style="width:7%; display:inline; float:left; text-align:center;">
							{$sort_blog_views}
						</div>
						...
						<div style="width:47%; display:inline; float:left;">
							{$sort_blog_title}
						</div>
				...
				{math equation="$mcount+1" assign="mcount"}
					<div class="{cycle  values="oddrow,evenrow"}">
						<div style="width:5%; display:inline; float:left; text-align:center;">
							<input type="checkbox" name="delete[]" value="{$item.id}" />
						</div>
						<div style="width:5%; text-align:center; display:inline; float:left;">
							{$mcount}
						</div>
						<div style="width:5%; text-align:center; display:inline; float:left;">
							{$item.views}
						...
						<div style="width:5%; display:inline; float:left; text-align:center;">
							{$item.votes} / {$item.num_votes}
						</div>
						<div style="width:50%; display:inline; float:left;">
							<a href="viewmyblog.php?id={$item.id}">{$item.short_title}</a>
						</div>
						<div style="width:15%; display:inline; float:left;">
				...
				{/foreach}
					<div align="left" class="outer_line">
			...
			{ else }
				<div class="outer-line">{lang mkey='no_blog_found'}</div>
			{/if}
	*}
