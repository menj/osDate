...
{/if}
{* <script type="text/javascript" src="{$smarty.const.DOC_ROOT}javascript/enlargeit.js"></script>  *}
<div style="vertical-align:top;" >
	{if $type == 'gallery'}
		...
		{if $userid == $smarty.session.UserId}
			{if $config.use_profilepopups == 'Y'}
				{assign var="page_hdr01_text_r" value='<a href="#" onclick="javascript:mainLink(\'uploadsnaps.php?type='|cat:$type|cat:'\'); window.close(); return false;" class="subhead">'|cat:"{lang mkey='upload_pictures'}"|cat:'</a>&nbsp;&nbsp;'}
			{else}
				{assign var="page_hdr01_text_r" value='<a href="uploadsnaps.php?type='|cat:$type|cat:'" class="subhead">'|cat:"{lang mkey='upload_pictures'}"|cat:'</a>&nbsp;&nbsp;'}
			{/if}
		{/if}
	{else}
		...
		{if $userid == $smarty.session.UserId}
			{if $config.use_profilepopups == 'Y'}
				{assign var="page_hdr01_text_r" value='<a href="#" onclick="javascript:mainLink(\'uploadsnaps.php?type='|cat:$type|cat:'\'); window.close(); return false;" class="subhead">'|cat:"{lang mkey='upload_profilepics'}"|cat:'</a>&nbsp;&nbsp;'}
			{else}
				{assign var="page_hdr01_text_r" value='<a href="uploadsnaps.php?type='|cat:$type|cat:'" class="subhead">'|cat:"{lang mkey='upload_profilepics'}"|cat:'</a>&nbsp;&nbsp;'}
			{/if}
		{/if}
	{/if}
	...
	{if $useralbums|@count > 0 }
		...
		{if $smarty.session.expired != 1 and $smarty.session.active == '1' and ( $smarty.session.status == $lang.status_enum.active or $smarty.session.status == 'Active') and $smarty.session.security.uploadpicture == 1  and $smarty.session.security.uploadpicturecnt > 0 and $userid == $smarty.session.UserId}
			{if $config.use_profilepopups == 'Y'}
				<a href="#" class="panellink" onclick=="javascript:mainLink(\'uploadsnaps.php?type={$type}\'); window.close(); return false;">{lang mkey='upload_pictures'}</a>
			{else}
				<a href="uploadsnaps.php?type={$type}" class="panellink">{lang mkey='upload_pictures'}</a>
			{/if}
		{/if}
		</div>
	    </form>
