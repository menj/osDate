{strip}
<div>
	{assign var="page_hdr01_text" value="{lang mkey='members_around'} "|cat:$smarty.session.iplocation.city_name|cat:", "|cat:$smarty.session.iplocation.state_name|cat:", "|cat:$smarty.session.iplocation.country_name|cat:"&nbsp;&nbsp;"|cat:$iplhdr02}
	{include file="page_hdr01.tpl"}
	<div class="module_detail_inside">
{if $iplusers|@count > 0}
		{assign var="ccount" value="0"}
	{if $config.iplocation_prof_display=='tiny'}
		{foreach item="item" key=key from=$iplusers}
		{if $ccount==0}
			<div>
		{/if}
